...
class View(ft.UserControl):
    ...
    def load_interface(self):
        ...
        row1 = ft.Row([self._txtAnno, self._btnCalcola], alignment=ft.MainAxisAlignment.CENTER, spacing=20)
        self._ddStato = ft.Dropdown(label="Stato", disabled=True)
        self._controller.fillStato()
        self._btnStatiRagg = ft.ElevatedButton(text="Stati raggiungibili", on_click=self._controller.handleStatiRagg, width = 200, disabled=True)
        row2 = ft.Row([self._ddStato, self._btnStatiRagg], alignment=ft.MainAxisAlignment.CENTER, spacing=20)
